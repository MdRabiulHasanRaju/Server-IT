...
if($_GET['type']=="income")
{
$aColumns = array( 's.id','s.stdid','s.paid','s.submitdate','s.transcation_remark');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "s.id";
	
	/* DB table to use */
	$sTable = "fees_transaction as s";
	...
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS   ".implode(", ", $aColumns)."
		FROM   ".$sTable." ORDER BY ID DESC ".$sLimit;
